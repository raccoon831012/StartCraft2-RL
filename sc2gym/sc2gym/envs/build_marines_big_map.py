...
CMD = {0:_SELECT_POINT,1:_BUILD_BARRACKS,2:_BUILD_SUPPLYDEPOT, 3:_TRAIN_MARINE, 4:_TRAIN_SCV, 5:_SELECT_IDLE_WORKER, 6:_MOVE_CAMERA}
class BuildMarinesBigMap1dEnv(BaseMovement2dEnv):
...
class BuildMarinesBigMap2dEnv(BuildMarinesBigMap1dEnv):
   def step(self, action):
      action = self._translate_action(action)
      obs, reward, done, info = self._safe_step(action)
      if obs is None:
