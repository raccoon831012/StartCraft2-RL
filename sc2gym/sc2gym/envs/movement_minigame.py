...
_MINIMAP_PLAYER_RELATIVE_SCALE = features.SCREEN_FEATURES.player_relative.scale
...
class BaseMovement2dEnv(BaseMovement1dEnv):
    def _get_action_space(self):
        ...
        return spaces.MultiDiscrete([83,83])
    def _translate_action(self, action):
...
class BaseMovement3dEnv(BaseMovement2dEnv):
    def step(self, action):
        action = self._translate_action(action)
        obs, reward, done, info = self._safe_step(action)
        if obs is None:
