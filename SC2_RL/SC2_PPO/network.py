...
class Network():
    def __init__(self, name: str, tf_screen_obs,tf_minimap_obs, act_space,sess, temp=0.1,):
        self.sess = sess
        """
        :param name: string
        :param env: gym env
        ...
        self.tf_screen_obs = tf_screen_obs
        self.tf_minimap_obs = tf_minimap_obs
        self.buildnet(name)
    ...
    def buildnet(self,name):
        with tf.variable_scope(name):
            ...
            with tf.variable_scope('policy_net'):
                self.CNN_output = tf.concat([self.CNN(self.tf_screen_obs),self.CNN(self.tf_minimap_obs)],axis=1)
